...
class Cookie(DessertItem):
    ...
    def combine(self, other: "Cookie") -> "Cookie":
        if not isinstance(other, Cookie):
            raise AttributeError("Can only combine Cookie with Cookie")
        if self.can_combine(other):
...
class IceCream(DessertItem):
    ...
    def __str__(self):
        ...
        return f"{self.name}, \
        {self.scoop_count} scoops, \
        ${self.price_per_scoop}/scoops, \
        ${self.calculate_cost()}, \
        {self.calculate_tax()}"
...
class Order(Payable):
    ...
    def sort(self) -> None:
        # Use Python's built-in sort function with a custom key
    ...
    def add(self, item: DessertItem) -> None:
        """If the self and item are the same class and have the same unit price, combine them.
        If they are not the same class, append the item to the order list"""
        if isinstance(item, (Candy, Cookie)):
            for existing_item in self.order:
                if isinstance(existing_item, type(item)) and existing_item.can_combine(item):
