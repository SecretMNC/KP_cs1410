...
def main():
  ...
  #
    ...
    for item in order:
        ...
        tax = f"[Tax: {item.calculate_tax():.2f}]"
        if isinstance(item, Candy):
            qty = item.candy_weight
            unit_price = item.price_per_pound
            receipt_list.append([name + f" ({item.packaging})"])
            receipt_list.append([f"    {qty} lbs. @ ${unit_price}/lb:", f"${cost:.2f}", f"{tax}"])
        elif isinstance(item, Cookie):
            qty = item.cookie_quantity
            unit_price = item.price_per_dozen
            receipt_list.append([name + f" ({item.packaging})"])
            receipt_list.append([f"    {qty} cookies @ ${unit_price}/dozen:", f"${cost:.2f}", f"{tax}"])
        elif isinstance(item, IceCream) and not isinstance(item, Sundae):
            qty = item.scoop_count
            unit_price = item.price_per_scoop
            receipt_list.append([name + f" ({item.packaging})"])
            receipt_list.append([f"    {qty} scoops @ ${unit_price}/scoop:", f"${cost:.2f}", f"{tax}"])
        elif isinstance(item, Sundae):
            qty = item.scoop_count
            unit_price = item.price_per_scoop
            receipt_list.append([name + f" ({item.packaging})"])
            receipt_list.append([f"    {qty} scoops @ ${unit_price}/scoop"])
            receipt_list.append([f"    {item.topping_name} cookies @ ${item.topping_price}:", f"${cost:.2f}", f"{tax}"])
            sundae_occurances += 1
    ...
    receipt_list.append([f"Paid with {order.get_pay_type()}"])

    

    make_receipt(receipt_list, "receipt.pdf", sundae_occurances)
